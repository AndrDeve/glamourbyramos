$(document).ready(function() {
  ...
  $('.comprar').click(function() {
    ...
    const precio = parseFloat(card.find('.precio').text().replace('C$', ''));
    const precio1 = card.find('.precio1').text();
    const total = precio;
  ...
  $('.comprar-articulos').click(function() {
    ...
    let message = `*Hola Glamour By Ramos*✨\n*Deseo comprar lo siguiente:*\n*Sección ${title}*\n\n`;
    let total = 0;

    cart.forEach(item => {
      message += `*-* ${item.referencia} - Nombre: ${item.nombre} - Precio: C$${item.precio1}\n\n`;
      total += item.total;
    });
  ...
  function updateCart() {
    ...
    cart.forEach(item => {
      ...
      tr.append(`<td>${item.precio1}</td>`);
      // tr.append(`<td>C$${item.precio.toFixed(2)}</td>`);
    ...
    $('.cart .total').text(`Total: C$${total.toFixed(2)}`);
  }
});
